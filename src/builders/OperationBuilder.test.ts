  ...
  describe("复杂场景测试", () => {
    it("应该支持所有方法的链式调用组合", () => {
      ...
      const result = builder
        ...
        .addResponse("200", successResponse)
        .addResponse("400", errorResponse)
        .addCallback("userUpdated", callback)
        .setDeprecated(false)
        .addSecurity(securityRequirement)
        .addServer(server)
        .addExtension("x-custom", "value")
        .addResponsesExtension("x-response-custom", "responseValue")
        .build();
      ...
      expect(result).toEqual({
        ...
        responses: {
          "200": successResponse,
          "400": errorResponse,
          "x-response-custom": "responseValue",
        },
        callbacks: {
    ...
    it("应该正确处理使用 Builder 方式添加的组件", () => {
      const builder = new OperationBuilder();
      const paramBuilder = new ParameterBuilder("search", "query").setDescription("搜索关键词");
      const requestBodyBuilder = new RequestBodyBuilder()
        .setDescription("请求体")
        .setRequired(true);
      const responseBuilder = new ResponseBuilder().setDescription("成功响应");
      ...
      const result = builder
        ...
        .addResponseFromBuilder("200", responseBuilder)
        .build();
