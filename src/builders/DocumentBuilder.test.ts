...
describe("DocumentBuilder", () => {
  describe("constructor and build", () => {
    ...
    it("应该正确验证必填字段", () => {
      ...
      expect(result.info.title).toStrictEqual("");
    ...
    it("应该保留用户提供的值而不覆盖", () => {
      ...
      expect(result.info.title).toStrictEqual("用户API");
      expect(result.info.version).toStrictEqual("2.0.0");
      expect(result.info.description).toStrictEqual("用户描述");
    });
  });
